...
.. _`Add DraftDocument`: https://developer.kontur.ru/doc/extern/method?type=post&path=%2Fv1%2F%7BaccountId%7D%2Fdrafts%2F%7BdraftId%7D%2Fdocuments
.. _`PUT DraftDocument`: https://developer.kontur.ru/doc/extern/method?type=put&path=%2Fv1%2F%7BaccountId%7D%2Fdrafts%2F%7BdraftId%7D%2Fdocuments%2F%7BdocumentId%7D
...
Черновик необходимо наполнить файлом отчета, приложениями к отчету и подписями. Главный файл отчета — это всегда xml-файл. К отчету могут идти также приложения и другие связанные документы, например, доверенность. Каждый файл в черновике нужно загрузить в сервис контенов методом :ref:`POST Upload<rst-markup-post-content>`, а также для каждого файла нужно создавать отдельный документ в черновике при помощи метода `Add DraftDocument`_. 

Чтобы изменить документ или положить новый контент файла используйте метод `PUT DraftDocument`_.  
...
Перед отправкой отчетности в налоговый орган необходимо вызвать для черновика три метода в строгом порядке: **Check -> Prepare -> Send**. Если хотя бы в одном из методов произошла ошибка, черновик не будет отправлен в налоговый орган. 
...
Методы, поддерживающие асинхронное выполнение, имеют флаг deferrer:

- Если флаг `deferrer = false` (по умолчанию), то вы будете ожидать выполнения операции, нужно держать соединение с сервером до завершения работы метода. 
- Если флаг `deferrer = true`, то метод будет выполняться асинхронно. Для выполнения метода будет создана задача (Task). Статус ее выполнения необходимо смотреть по taskId, который веренется в ответе метода. 
